...
// @Summary Add authors to category
// @Tags categories
// @Accept json
// @Produce json
...
func (t *categoriesService) AddAuthor(ctx *gin.Context) {
	// Получаем параметр id из запроса
	idParam, existId := ctx.Params.Get("id")
	id, errAtoi := strconv.Atoi(idParam)

	if !existId || errAtoi != nil {
		logger.Log.Error(fmt.Sprintf("AddAuthor: Parameter id exist - %t, error: %v", existId, errAtoi))
		sendResponse.Send(
			ctx,
	...
	if errBindJSON := ctx.BindJSON(body); errBindJSON != nil {
		logger.Log.Error(fmt.Sprintf("AddAuthor: %v", errBindJSON.Error()))
		sendResponse.Send(
			ctx,
			http.StatusBadRequest,
			"error",
	...
	if errCheckExistCategory != nil {
		logger.Log.Error(fmt.Sprintf("AddAuthor: %v", errCheckExistCategory.Error()))
		sendResponse.Send(
			ctx,
	...
	for i := 0; i < len(body.AuthorsIds); i++ {
		...
		if errCheckExistAuthor != nil {
			logger.Log.Error(fmt.Sprintf("AddAuthor: %v", errCheckExistAuthor.Error()))
			sendResponse.Send(
				ctx,
				http.StatusInternalServerError,
				"error",
		...
		// Добавляем автора в категорию
		if errAddAuthor := t.repository.AddAuthor(authorId, id); errAddAuthor != nil {
			logger.Log.Error(fmt.Sprintf("AddAuthor: %v", errAddAuthor.Error()))
			sendResponse.Send(
				ctx,
				http.StatusInternalServerError,
				"error",
